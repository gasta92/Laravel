                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                              @forelse ($kategori as $v)
                                <tr>
                                    <td width="80%">{{ $v->nama }}</td>
                                    <td width="20%" align="center" class="text-center">
                                        <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('kategori.destroy', $v->id) }}" method="POST">
                                            <a href="{{ route('kategori.edit', $v->id) }}" class="btn btn-sm btn-primary">EDIT</a>
                                            @csrf
                                            @method('DELETE')
